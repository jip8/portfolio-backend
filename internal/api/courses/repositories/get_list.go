...
func (r *GetListRepository) Execute(ctx context.Context, listReq entity.ListReq) (*entity.List[entity.CourseResp], error) {
	var items []entity.CourseResp
	query := `
	SELECT
		id,
		title,
	...
	var total int
	if err := executor.GetContext(ctx, &total, `SELECT COUNT(*) FROM portfolio.courses`); err != nil {
		return nil, err
	}
	...
	if len(items) > 0 {
		...
		items = make([]entity.CourseResp, 0)
	}

	return &entity.List[entity.CourseResp]{
