...
func (r *GetListRepository) Execute(ctx context.Context, listReq entity.ListReq) (*entity.List[entity.ProjectResp], error) {
	...
	var total int
	if err := executor.GetContext(ctx, &total, `SELECT COUNT(*) FROM portfolio.projects`); err != nil {
		return nil, err
	}
	...
	if len(items) > 0 {
		...
		items = make([]entity.ProjectResp, 0)
	}

	return &entity.List[entity.ProjectResp]{
