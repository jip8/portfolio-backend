...
func (r *CreateRepository) Execute(ctx context.Context, req entity.ProjectFlat) (*int, error) {
	query := `
		INSERT INTO portfolio.projects 
			(title, description, published_at, revelance)
		VALUES 
			(:title, :description, :published_at_time, :revelance)
		RETURNING id
	`
